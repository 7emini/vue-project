    ...
    >
      <el-form-item label="信息类别:" prop="category">
        <el-cascader
          ...
          :options="category_data.category_options"
...
function handlerSubmitForm() {
  formDom.value.validate((valid) => {
    if (valid) {
      ...
      requestData.create_date = dayjs(requestData.create_date).format(
        "YYYY-MM-DD HH:mm:ss"
      );
      requestData.category_id =
        requestData.category_id[requestData.category_id.length - 1];
      console.log(requestData);
      infoCreate(requestData).then(response=>{
